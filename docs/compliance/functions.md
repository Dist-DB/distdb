...
| Window function usage in SELECT projections | Partial | `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `SUM`, `AVG`, `MIN`, and `MAX` window projections execute in relation/join SELECT paths, including named-window reuse | Coverage is intentionally limited to the current window function set and frame units |
...
- Window-function execution slices in SELECT projections:
  ...
  - `RANK() OVER (...)` and `DENSE_RANK() OVER (...)`
  - `SUM(<column>)`, `AVG(<column>)`, `MIN(<column>)`, and `MAX(<column>)` over direct and named-window specs
...
- Window-function coverage is currently limited to `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `SUM`, `AVG`, `MIN`, and `MAX`; broader analytic/window parity is not implemented yet.
