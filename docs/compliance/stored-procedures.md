...
| CALL execution path | Supported | Server query mapping resolves procedure, binds args, invokes routine, returns action result | Invocation-scoped cleanup is enforced after execution; cleanup failures are surfaced |
...
| CALL argument expression shapes | Partial | Supports literals (boolean/number/string/NULL), signed numeric unary, identifier and compound identifier forms, and `?` placeholder token passthrough | Subquery args and many general expressions are rejected |
...
| Procedure-local temporary tables | Supported | Temporary table creation in CALL actions is scoped, aliased, and cleaned up | Name rewrite maps logical names to scoped physical IDs during action execution; scoped resources are removed from catalog visibility after cleanup |
