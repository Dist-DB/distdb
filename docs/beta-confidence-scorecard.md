...
| Operability/upgrade safety | Yellow | 2026-07-17 | server/serverlib | rolling restart and cross-version/rollback drill evidence now runs as a maintained compatibility-window matrix (`HEAD~1`, `HEAD~2`, `HEAD~3`), frozen WAL/catalog window assertions are published, and per-window history depth is now enforced in nightly evidence gating (`scripts/e2e/rolling_restart_upgrade_safety.sh`, `scripts/append_artifact_trends.sh`, `scripts/check_artifact_evidence_quality.sh`, `.github/workflows/operability-upgrade-safety.yml`, `.github/workflows/nightly-evidence.yml`, `docs/operability-upgrade-safety.md`, `docs/operability-runbooks.md`); remaining blocker is accumulating enough passing scheduled runs and keeping high-severity operability regressions closed |
