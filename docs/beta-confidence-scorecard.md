...
| Non-functional behavior | Red | 2026-07-17 | server/serverlib | benchmark baselines now defined; trend baselines, thresholds, and regression budgets still required |
| Security/adversarial validation | Yellow | 2026-07-17 | server/serverlib | baseline matrix and findings governance are in place with transport/trust/replay plus authorization interleaving fault-injection coverage; extend to higher-contention race/parallel abuse depth and keep high-severity findings closed |
| Operability/upgrade safety | Red | 2026-07-17 | server/serverlib | rolling upgrade evidence, format compatibility contract, runbook drills |
