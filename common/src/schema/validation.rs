...
impl std::fmt::Display for SchemaValidationError {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            Self::InvalidBitWidth => write!(f, "numeric bit width must be one of 8, 16, 32, 64"),

            Self::InvalidFixedStringLen => write!(f, "fixed-length string size must be greater than zero"),
            
            Self::EmptyEnumVariants => write!(f, "enum field must define at least one variant"),

            Self::EmptyEnumVariantValue => write!(f, "enum variants must not contain empty values"),
