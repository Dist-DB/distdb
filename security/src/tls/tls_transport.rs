...
async fn probe_plaintext_bootstrap_frame(
    ...
    loop {
        let bytes_peeked = match timeout(Duration::from_millis(25), stream.peek(&mut probe)).await {
            Ok(Ok(bytes)) => bytes,
...
pub async fn negotiate_connector_stream(
    ...
    match tls_mode {
        ...
        common::TlsMode::Required => {
            ...
            if let Some(is_bootstrap_frame) =
                probe_plaintext_bootstrap_frame(&stream, REQUIRED_TLS_BOOTSTRAP_PROBE_TIMEOUT_MS)
                    .await?
                && is_bootstrap_frame
            {
        ...
        common::TlsMode::Optional => {
            ...
            let mut probe = [0u8; 8];
            let bytes_peeked =
                match timeout(Duration::from_millis(OPTIONAL_TLS_PROBE_TIMEOUT_MS), stream.peek(&mut probe)).await {
                    Ok(Ok(bytes)) => bytes,
